    ...
    class Program
    {
        public static void Main(string[] args)
        {
            ...
            login:
                ...
                Employee check = pharmacy.employees.Find(u => u.Username.ToUpper() == username.ToUpper() && u.Password == password);
                if (check == null)
                {
                ...
                {
                    if (item.Username==username&& item.Password==password&& item.RoleType == Role.Admin)
                    {
                    ...
                    Main:
                        ...
                        switch (choose)
                        {
                            ...
                            case "2":
                                ...
                                break;
                            case "3":
                                UpdateData(pharmacy.employees, pharmacy);
                                break;
                            case "4":
                                goto login;
                            default:
                    ...
                    {
                        Staff();
                        string choosestaff = Console.ReadLine();
        ...
        static void Admin()
        {
            Helper.PrintSimple("-------------------ADMIN-----------", ConsoleColor.Cyan);
            ...
            Helper.PrintSimple("4: Log out  ", ConsoleColor.Blue);
        ...
        static void Staff()
        {
            ...
            Helper.PrintSimple("-------------------Staff-----------", ConsoleColor.Cyan);
            Helper.PrintSimple("1: Sell ", ConsoleColor.Blue);
            Helper.PrintSimple("2:Update user data  ", ConsoleColor.Blue);
            Helper.PrintSimple("3:Log out  ", ConsoleColor.Blue);
        ...
        SalePrice:
            ...
            pharm.Budget -= count * purchasePrice;
        ...
        username:
            ...
            foreach (var item in pharm.employees)
            {
                if (item.Username == empUsername)
                {
                    Helper.SuccessAndError($"{item.Username}  has already exist. Enter another username ", ConsoleColor.DarkRed);
                    goto username;
                }
            }
        ...
        select:
            ...
            //bool existempName = employees.Any(x => x.Username == emp.Username);
            //if (existempName)
            //{
            //    Helper.SuccessAndError($"{emp.Name}  has already exist. Enter another username ", ConsoleColor.DarkRed);
            //    goto select;
            //}
        ...
        Salary:
            ...
            Helper.SuccessAndError("This empployee is added",ConsoleColor.Green);
        ...
        {
            {
                if (drugs.Count == 0)
                {
                    Helper.PrintSimple("Drug list is empty: ", ConsoleColor.Red);
                    return;
                }
            salename:
                Helper.PrintSimple("Enter drug name for sale : ", ConsoleColor.Blue);
                string drugname = Console.ReadLine();
                List<Drug> searchdrug = drugs.FindAll(x => x.Name.ToUpper().Contains(drugname.ToUpper()));
                if (searchdrug.Count == 0)
                {
                    Helper.PrintSimple("Drugs can't found: ", ConsoleColor.Blue);
                    goto salename;
                }
                ...
                foreach (var item in searchdrug)
                {
                    Helper.PrintSimple($"Name:{item.Name} type:{item.DrugType} count:{item.Count} price:{item.SalePrice}", ConsoleColor.Blue);
                }
            chosSaletype:
                Helper.PrintSimple("Choose drug type:\n1)Syrop 2) Powder 3) Tablet", ConsoleColor.Blue);
                ...
                switch (chosSaletype)
                {
                    case "1":
                        foreach (var item in searchdrug)
                        {
                            ...
                            {
                                Helper.PrintSimple("Enter name for sale : ", ConsoleColor.Cyan);
                                string salenameStr = Console.ReadLine();
                                if (item.Name.ToUpper() == salenameStr.ToUpper())
                                {
                                saleCount:
                                    Helper.PrintSimple("Enter count for sale : ", ConsoleColor.Cyan);
                                    string saleCountStr = Console.ReadLine();
                                    bool isSalCount = int.TryParse(saleCountStr, out int saleCount);
                                    if (!isSalCount)
                                    {
                                    ...
                                    if (item.Count < saleCount)
                                    {
                                        ...
                                        if (answer1.ToUpper() == "Y")
                                        {
                                          ...
                                          drugs.Remove(item);
                                            Helper.SuccessAndError("Drugs are sold", ConsoleColor.Green);
                                            break;
                                        }
                                        else
                                        {
                                    ...
                                    else
                                    {
                                        ...
                                        if (sureAnswer.ToUpper() == "Y")
                                        {
                                            ...
                                            Helper.SuccessAndError("Drugs are sold", ConsoleColor.Red);
                                            break;
                                        }
                                        else
                                        {
                            ...
                            {
                                Helper.SuccessAndError("Drug can't found", ConsoleColor.Red);
                                goto chosSaletype;
                            }
                        }
                        break;
        ...
        static void UpdateData(List<Employee> emp, Pharmacy pha)
            {


            empName:
                Helper.PrintSimple("Enter employee new name : ", ConsoleColor.DarkMagenta);
                string emplName = Console.ReadLine();
                bool isemplName = string.IsNullOrWhiteSpace(emplName);
                if (isemplName)
                {
            ...
            empsurname:
                Helper.PrintSimple("Enter employee new surname : ", ConsoleColor.DarkMagenta);
                string editSurname = Console.ReadLine();
                bool isedSurname = string.IsNullOrWhiteSpace(editSurname);
                if (isedSurname)
                {
            ...
            empusername:
                Helper.PrintSimple("Enter employee new username : ", ConsoleColor.DarkMagenta);
                string edUsername = Console.ReadLine();
                bool isedUsername = string.IsNullOrWhiteSpace(edUsername);
                if (isedUsername)
                {
                ...
                bool existedUsernam = emp.Any(x => x.Username == edUsername);
                if (existedUsernam)
                {
                    Helper.SuccessAndError("This username already exist ! Please try another username ", ConsoleColor.DarkRed);
                    goto empusername;
                }
            edpassword:
                Helper.PrintSimple("Enter password", ConsoleColor.DarkCyan);
            ...
            edbirthDay:
                ...
                {
                    item.Name = emplName;
                    item.Surname = editSurname;
                    item.Username = edUsername;
                    item.BirthDate = editbirthDay;
                item.Password = edpassword;


                }
                Helper.SuccessAndError("Data is successfully update! ", ConsoleColor.Green);
